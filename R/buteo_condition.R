  ...
  if (.plot == T) {
    ...
    out <- lapply(1:nrow(df), function(x) {
      ...
      if (any(is.na(df[x, c("sex", "weight", "wing")]))) {
        ...
        return(df[x, "weight"] - stats::predict.lm(male.lm, df[x,], se.fit = F)[[1]])
      } else {
        return(df[x, "weight"] - stats::predict.lm(female.lm, df[x,], se.fit = F)[[1]])
      }
